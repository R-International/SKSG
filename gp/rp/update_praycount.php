...
if (!empty($phone_number) && !empty($cur_date) && is_numeric($pray_count) && $pray_count<=421) {
    ...
    $check_sql = "SELECT * FROM pray_counter1 WHERE phone_number = '$phone_number' AND cur_date = '$cur_date'";
    $result = mysqli_query($con, $check_sql);

    if (mysqli_num_rows($result) > 0) {
        // Update the existing record
        $update_sql = "UPDATE pray_counter1 SET prayCounter = $pray_count
        ...
        // Insert a new record
        $insert_sql = "INSERT INTO pray_counter1 (phone_number, prayCounter, cur_date)
