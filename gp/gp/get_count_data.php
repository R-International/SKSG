...
if ($registerResult->num_rows > 0) {
    while ($registerRow = $registerResult->fetch_assoc()) {
        ...
        // Fetch sum of prayCounter from pray_counter table using RIGHT JOIN
        $prayCounterQuery = "SELECT SUM(prayCounter) AS totalPrayCounter FROM pray_counter WHERE phone_number = ?";
